...
const manageSecondaryNav = (primary, secondary) => {
...
const manageActiveClass = (primary, secondary) => {
...
const checkIfInCollection = (tmdbId) => {
...
const updateList = (tmdbId, id) => {
   ...
   // IF LISTS EXIST
      ...
      // ITERATE OVER ARRAY AND ASSIGN DIFFERENT CALLBACKS FOR TRUE AND FALSE
...
const addNewList = (divId, inputId) => {
...
// RENDER AGAIN
const addItemToList = (list, tmdbId, id) => {
...
const deleteItemFromList = (list, tmdbId, id, remove) => {
...
const showContentResults = (results) => {
   ...
   results.map((result) => {
      ...
      if (result.hasOwnProperty('adult')) {
         genreType = state.movieGenres;
         mediaType = 'movie';
      } else {
         genreType = state.tvGenres;
         mediaType = 'tv';
      }
...
const showFullMediaContent = (result) => {
   // EXTRACT RESULTS & SET BACKUP IF FAILURE
   const tmdbId = result.id || '0';
   const title = result.title || result.name || 'Unknown';
   const tagline =
      result.tagline ||
      `NO. SEASONS: ${result.number_of_seasons}  ~  NO. EPISODES: ${result.number_of_episodes}` ||
      '';
...
const pagination = (primary, secondary, totalPages, page) => {
   resetPagination();
   ...
   while (start + i <= end) {
      mainPagination.innerHTML += `
            <span class="pagination-box" onclick="fetchTMDbData('${primary}','${secondary}',${
         start + i
      })">${start + i}</span>
        `;
      i++;
...
const checkIfCollectionChanged = (tmdbId) => {
...
const mylist = {
   movies: [
      ...
      },
      {
         ...
         genre_ids: [28, 12, 878, 10749, 35, 10751],
...
let examData = JSON.stringify(mylist);
